...
st.markdown("""
<style>
.block-container {padding-top: 1rem; padding-bottom: 1rem; padding-left: 1.4rem; padding-right: 1.4rem; max-width: 100%;}
section[data-testid="stSidebar"] > div {background: #09111f;}
.hero {padding: 1.2rem 1.35rem; border-radius: 18px; background: linear-gradient(135deg, #0b1220 0%, #101a2d 100%); border: 1px solid #1e293b; margin-bottom: 1rem;}
.hero h1 {font-size: 2.2rem; font-weight: 800; color: #eef4ff; margin: 0;}
.hero p {font-size: 1rem; color: #9fb0c7; margin: 0.4rem 0 0 0;}
.pillrow {display:flex; gap:0.45rem; flex-wrap:wrap; margin-top:0.8rem;}
.pill {padding:0.3rem 0.65rem; border-radius:999px; background:#122033; color:#9cd0ff; font-size:0.8rem; border:1px solid #22354e;}
.card {background: linear-gradient(180deg, #0b1220 0%, #0f172a 100%); border: 1px solid #1f2d42; border-radius: 16px; padding: 1rem;}
...
.section-title {font-size:1.15rem; font-weight:700; color:#eef4ff; margin:0.2rem 0 0.9rem 0;}
...
def load_data(uploaded_file=None):
    ...
    df["Wealth Score"] = (0.45*df["Income"] + 5*df["CCAvg"] + 0.05*df["Mortgage"] + 8*df["Education"]).round(1)
    return df
...
def simple_score(df):
    ...
    score = (
        1.15*z_income + 0.95*z_cc + 0.35*df["Education"] + 0.22*df["CD Account"] +
        0.10*df["Securities Account"] + 0.08*df["Online"] + 0.10*df["CreditCard"] +
        0.18*df["Family"] + 0.18*z_mort - 0.12*df["Age"] / 100
    )
    score = 1 / (1 + np.exp(-np.clip(score, -12, 12)))
    return score
...
<div class="hero">
  ...
  <p>Fresh client-ready version focused on loan conversion, segment behavior, and cross-sell actions.</p>
  <div class="pillrow">
    <div class="pill">Personal Loan</div>
    <div class="pill">Segment View</div>
    <div class="pill">Model Comparison</div>
...
items = [
    ...
    ("Avg Wealth Score", f"{wealth_avg:.1f}", "Composite affluence indicator"),
]
for col, (label, value, note) in zip([k1,k2,k3,k4,k5], items):
    col.markdown(f"<div class='card'><div class='label'>{label}</div><div class='value'>{value}</div><div class='note'>{note}</div></div>", unsafe_allow_html=True)
...
with t1:
    ...
    with b:
        ...
        st.markdown(f"<div class='insight'><b>Best education segment:</b> {edu.iloc[0,0]} with {edu.iloc[0,-1]:.1f}% acceptance.<br><br><b>Best income segment:</b> {inc.iloc[0,0]} with {inc.iloc[0,-1]:.1f}% acceptance.<br><br><b>Best model:</b> {best_name} with ROC AUC {metrics.iloc[0]['ROC AUC']:.3f}.</div>", unsafe_allow_html=True)
        st.dataframe(metrics, use_container_width=True, hide_index=True)

    a, b = st.columns(2, gap="large")
    with a:
        st.markdown("<div class='section-title'>Acceptance by Education</div>", unsafe_allow_html=True)
        edu_chart = acceptance_by(view, "Education Label").set_index("Education Label")[["Acceptance Rate %"]]
        st.bar_chart(edu_chart)
    with b:
        st.markdown("<div class='section-title'>Acceptance by Income Band</div>", unsafe_allow_html=True)
        inc_chart = acceptance_by(view, "Income Band").set_index("Income Band")[["Acceptance Rate %"]]
        st.bar_chart(inc_chart)

with t2:
    a, b = st.columns(2, gap="large")
    with a:
        st.markdown("<div class='section-title'>Acceptance by Age Band</div>", unsafe_allow_html=True)
    ...
    with b:
        st.markdown("<div class='section-title'>Acceptance by Credit Spend</div>", unsafe_allow_html=True)
    ...
    matrix = pd.DataFrame(rows).sort_values("Lift %", ascending=False)
...
with t3:
    ...
    with a:
        ...
        st.dataframe(metrics, use_container_width=True, hide_index=True)
        st.markdown(f"<div class='insight'><b>{best_name}</b> is the recommended model for presentation because it gives the strongest ranking quality in this lightweight version.</div>", unsafe_allow_html=True)
    with b:
        st.markdown("<div class='section-title'>Confusion Matrix</div>", unsafe_allow_html=True)
        cm = confusion_df(df[TARGET].to_numpy(int), df["Predicted Class"].to_numpy(int))
        st.dataframe(cm, use_container_width=True)

    score_drivers = pd.Series({
        "Income": 1.15,
        "CCAvg": 0.95,
        "Education": 0.35,
        "CD Account": 0.22,
    ...
    st.bar_chart(score_drivers)
    driver_df = score_drivers.reset_index()
    driver_df.columns = ["Feature", "Relative Weight"]
...
with t4:
    ...
    with a:
        ...
        st.bar_chart(p)
        st.dataframe(p.reset_index().rename(columns={"index":"Persona"}), use_container_width=True, hide_index=True)
    with b:
...
st.caption("Deployment-safe version: no Plotly import, so the previous plotly error is fully removed.")
